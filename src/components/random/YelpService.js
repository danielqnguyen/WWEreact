...
class YelpApi {
  static yelpTen(food, location, dollars, onSuccess, onError) {
    ...
    axios.get(`https://api.yelp.com/v3/businesses/search?term=${food}&open_now=true&location=${location}&price=${dollars}`, { headers: config })
      .then(onSuccess)
      .catch(onError);
  }
  ...
  static yelpById(id, onSuccess, onError) {
    ...
    axios.get(`https://api.yelp.com/v3/businesses/${id}`, { headers: config })
      .then(onSuccess)
      .catch(onError);
  }
}
